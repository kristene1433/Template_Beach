...
const AdminDashboard = () => {
  ...
  const handleLeaseFileUpload = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Update the selected application with the new file info
        setSelectedApplication(prev => ({
          ...prev,
          signedLeaseFile: data.signedLeaseFile
        }));
        ...
        const error = await response.json();
        toast.error(error.error || 'Failed to upload lease file');
      }
    } catch (error) {
      console.error('Error uploading lease file:', error);
      toast.error('Failed to upload lease file');
    } finally {
              ...
              {(selectedApplication.leaseStartDate || selectedApplication.leaseEndDate || selectedApplication.rentalAmount) && (
                <div className="mt-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {selectedApplication.signedLeaseFile && (
                      <div className="flex items-center justify-between">
                        ...
                        <a
                          href={`/api/application/admin/lease-file/${selectedApplication.signedLeaseFile.filename}`}
                          target="_blank"
                          rel="noopener noreferrer"
                  ...
                  <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <h5 className="text-sm font-medium text-gray-900 mb-3">Upload Signed Lease</h5>
                    <p className="text-xs text-gray-600 mb-3">
                      Upload the physically signed lease agreement that was mailed by the tenant.
                    </p>
                    
                    <form onSubmit={handleLeaseFileUpload} className="space-y-3">
                      <div>
                        <input
                          id="lease-file-input"
                          type="file"
                      ...
                      {leaseFile && (
                        <div className="flex items-center text-sm text-gray-600">
                          ...
                          Selected: {leaseFile.name} ({(leaseFile.size / 1024 / 1024).toFixed(2)} MB)
                        </div>
                      )}
              ...
              <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className="inline-flex items-center px-3 py-2 text-sm font-medium text-purple-700 bg-purple-100 border border-purple-300 rounded-md hover:bg-purple-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500"
                  >
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Add check payments, remove incorrect entries, or transfer amounts between applications.
                </p>
              </div>
